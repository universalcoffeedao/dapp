...
import { wsCalmContract } from "../../calmAbi";
...
export const changeApproval = createAsyncThunk("wrapping/changeApproval", async ({ provider, address, networkID }: IChangeApproval, { dispatch }) => {
  ...
  const sCalmContract = new ethers.Contract(addresses.SCALM_ADDRESS, wsCalmContract, signer);

  let approveTx;
  try {
    const gasPrice = await getGasPrice(provider);

    approveTx = await sCalmContract.approve(addresses.WSCALM_ADDRESS, ethers.constants.MaxUint256, { gasPrice });
  ...
  const wsCalmAllowance = await sCalmContract.allowance(address, addresses.WSCALM_ADDRESS);

  return dispatch(
    fetchAccountSuccess({
      wrapping: {
        wsCalm: Number(wsCalmAllowance),
      },
    }),
  );
});
...
export const changeWrap = createAsyncThunk("wrapping/changeWrap", async ({ isWrap, value, provider, networkID, address }: IChangeWrap, { dispatch }) => {
  ...
  const wscalmContract = new ethers.Contract(addresses.WSCALM_ADDRESS, wsCalmContract, signer);
  ...
  try {
    ...
    if (isWrap) {
      wrapTx = await wscalmContract.wrap(amountInWei, { gasPrice });
    } else {
      wrapTx = await wscalmContract.unwrap(amountInWei, { gasPrice });
    }
...
const calcWrapValue = async ({ isWrap, value, provider, networkID }: IWrapDetails): Promise<number> => {
  ...
  if (isWrap) {
    const wsCalmValue = await wscalmContract.sCALMTowsCALM(amountInWei);
    wrapValue = wsCalmValue / Math.pow(10, 18);
  } else {
    const sCalmValue = await wscalmContract.wsCALMTosCALM(amountInWei);
    wrapValue = sCalmValue / Math.pow(10, 9);
  }
