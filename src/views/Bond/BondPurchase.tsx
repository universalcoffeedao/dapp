    ...
    <Box display="flex" flexDirection="column">
      <Box display="flex" justifyContent="space-around" flexWrap="wrap">
        ...
        <FormControl className="bond-input-wrap" variant="outlined" color="primary" fullWidth>
          <OutlinedInput
            placeholder={`${bond.displayName} Amount`}
            type="number"
            value={quantity}
        ...
        {hasAllowance() || useAvax ? (
          <div
            ...
            style={{ backgroundColor: isPendingTxn(pendingTransactions, "bond_" + bond.name) ? "#007500" : "rgba(255, 255, 255, 0.2)" }}
          >
            <p style={{ color: "#ffc768" }}>{txnButtonText(pendingTransactions, "bond_" + bond.name, "Mint CALM")}</p>
          </div>
        ) : (
          <div
            className="transaction-button bond-approve-btn"
          ...
          >
            <p style={{ color: "#ffc768" }}>{txnButtonText(pendingTransactions, "approve_" + bond.name, "Approve")}</p>
          </div>
        )}
      ...
      <Slide direction="left" in={true} mountOnEnter unmountOnExit {...{ timeout: 533 }}>
        <Box className="bond-data">
          ...
          <div className="data-row">
            <p className="bond-balance-title">You Will Spend</p>
            <p className="price-data bond-balance-title">
              {isBondLoading ? <Skeleton width="100px" /> : quantity === "" ? `0 ${bond.displayName}` : `${quantity} ${bond.displayName}`}
            </p>
          </div>

          <div className="data-row">
            <p className="bond-balance-title">You Will Get</p>
            <p className="price-data bond-balance-title">{isBondLoading ? <Skeleton width="100px" /> : `${trim(bond.bondQuote, 4)} CALM`}</p>
          </div>

          <div className={`data-row`}>
            <p className="bond-balance-title">Max You Can Buy</p>
            <p className="price-data bond-balance-title">{isBondLoading ? <Skeleton width="100px" /> : `${trim(bond.maxBondPrice, 4)} CALM`}</p>
          </div>
          ...
          <div className="data-row">
            ...
            <p className="bond-balance-title">0.01 CALM</p>
          </div>
        </Box>
      </Slide>
