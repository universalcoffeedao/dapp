    ...
    <Box display="flex" flexDirection="column">
      <Box display="flex" justifyContent="space-around" flexWrap="wrap">
        <div
          ...
          style={{ backgroundColor: isPendingTxn(pendingTransactions, "redeem_bond_" + bond.name) ? "#007500" : "rgba(255, 255, 255, 0.2)" }}
        >
          <p style={{ color: "#ffc768" }}>{txnButtonText(pendingTransactions, "redeem_bond_" + bond.name, "Claim")}</p>
        </div>
        <div
          className="transaction-button bond-approve-btn"
          onClick={() => {
          ...
          style={{ backgroundColor: isPendingTxn(pendingTransactions, "redeem_bond_" + bond.name + "_autostake") ? "#007500" : "rgba(255, 255, 255, 0.2)" }}
        >
          <p style={{ color: "#ffc768" }}>{txnButtonText(pendingTransactions, "redeem_bond_" + bond.name + "_autostake", "Claim and Autostake")}</p>
        </div>
      </Box>
      ...
      <Slide direction="right" in={true} mountOnEnter unmountOnExit {...{ timeout: 533 }}>
        <Box className="bond-data">
          <div className="data-row">
            ...
            <p className="price-data bond-balance-title">{isBondLoading ? <Skeleton width="100px" /> : `${trim(bond.interestDue, 4)} CALM`}</p>
          </div>
          <div className="data-row">
            <p className="bond-balance-title">Claimable Rewards</p>
            <p className="price-data bond-balance-title">{isBondLoading ? <Skeleton width="100px" /> : `${trim(bond.pendingPayout, 4)} CALM`}</p>
          </div>
          <div className="data-row">
