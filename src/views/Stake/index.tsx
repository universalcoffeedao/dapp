...
function Stake() {
  ...
  const stakeAllowance = useSelector<IReduxState, number>(state => {
    return state.account.staking && state.account.staking.calm;
  });
  const unstakeAllowance = useSelector<IReduxState, number>(state => {
    return state.account.staking && state.account.staking.scalm;
  });
  ...
  const setMax = () => {
    if (view === 0) {
      setQuantity(calmBalance);
    } else {
      setQuantity(scalmBalance);
    }
  };
  ...
  const hasAllowance = useCallback(
    token => {
      if (token === "calm") return stakeAllowance > 0;
      if (token === "scalm") return unstakeAllowance > 0;
      return 0;
    },
    [stakeAllowance],
  );
  ...
  const trimmedsCalmBalance = trim(Number(scalmBalance), 6);
  const trimmedStakingAPY = trim(stakingAPY * 100, 1);
  const stakingRebasePercentage = trim(stakingRebase * 100, 4);
  const nextRewardValue = trim((Number(stakingRebasePercentage) / 100) * Number(trimmedsCalmBalance), 6);
  return (
          ...
          <Grid className="stake-card-grid" container direction="column" spacing={2}>
            <Grid item>
              <div className="stake-card-header">
                <p className="stake-card-header-title">CALM Staking (☮️, ☮️)</p>
                <RebaseTimer />
              </div>
            </Grid>
                ...
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={6} sm={4} md={4} lg={4}>
                    <div className="stake-card-index">
                      ...
                      <p className="stake-card-metrics-value">{currentIndex ? <>{trim(Number(currentIndex), 2)} CALM</> : <Skeleton width="150px" />}</p>
                    </div>
                  </Grid>
                </Grid>
              </div>
                ...
                <div>
                  <div className="stake-card-action-area">
                      ...
                      {view === 0 && (
                        <div className="stake-card-tab-panel">
                          {address && hasAllowance("calm") ? (
                            ...
                            >
                              <p>{txnButtonText(pendingTransactions, "staking", "Stake CALM")}</p>
                            </div>
                          ) : (
                            <div
                              className="stake-card-tab-panel-btn"
                      ...
                      {view === 1 && (
                        <div className="stake-card-tab-panel">
                          {address && hasAllowance("scalm") ? (
                            ...
                            >
                              <p>{txnButtonText(pendingTransactions, "unstaking", "Unstake CALM")}</p>
                            </div>
                          ) : (
                            <div
                              className="stake-card-tab-panel-btn"
                    ...
                    <div className="stake-card-action-help-text">
                      {address && ((!hasAllowance("calm") && view === 0) || (!hasAllowance("scalm") && view === 1)) && (
                        <p>
                          Note: The "Approve" transaction is only needed when staking/unstaking for the first time; subsequent staking/unstaking only requires you to perform the
                  ...
                  <div className="stake-user-data">
                    <div className="data-row">
                      ...
                      <p className="data-row-value">{isAppLoading ? <Skeleton width="80px" /> : <>{trim(Number(calmBalance), 4)} CALM</>}</p>
                    </div>
